...
if [[ "$FILTER_COUNT" -eq 0 ]]; then
  ...
  curl -X POST -H "Accept: application/vnd.github+json" -H "Authorization: token $GITHUB_API_TOKEN" "https://api.github.com/repos/ergeon/$CIRCLE_PROJECT_REPONAME/statuses/$CIRCLE_SHA1" -d '{"state":"success","description":"Coverage passed!","context":"coverage/coveralls"}'
fi
